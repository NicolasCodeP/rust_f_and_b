...
pub fn show_soft_input(show_implicit: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let jni_window = env.call_method(&activity, "getWindow", "()Landroid/view/Window;", &[])?
        .l()?;
    if env.exception_check()? {
        env.exception_clear()?;
    ...
    let view = env.call_method(&jni_window, "getDecorView", "()Landroid/view/View;", &[])?
        .l()?;
    if env.exception_check()? {
        env.exception_clear()?;
...
pub fn hide_soft_input() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let decor_view = env.call_method(&window, "getDecorView", "()Landroid/view/View;", &[])?
        .l()?;
    let window_token = env.call_method(&decor_view, "getWindowToken", "()Landroid/os/IBinder;", &[])?
        .l()?;
